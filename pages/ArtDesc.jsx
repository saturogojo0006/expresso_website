...
const ArtDesc = () => {
  ...
  const currentArt = artKeys[toggle];
  const artInfo = artData[currentArt];
  const [artName, setArtName] = useState(artInfo.artName);
  const [artImage, setArtImage] = useState(artInfo.artImage);
  const [description, setDescription] = useState(artInfo.description);
  const [artist, setArtist] = useState(artInfo.artist);

  const handleButtonLeft = () => {
    setToggle((prevIndex) => (prevIndex - 1 + artKeys.length) % artKeys.length);
  };

  const handleButtonRight = () => {
    setToggle((prevIndex) => (prevIndex + 1) % artKeys.length);
  };

  // Update art information when toggle changes
  useEffect(() => {
    const newArt = artKeys[toggle];
    const newArtInfo = artData[newArt];
    setArtName(newArtInfo.artName);
    setArtImage(newArtInfo.artImage);
    setDescription(newArtInfo.description);
    setArtist(newArtInfo.artist);
  }, [toggle, artKeys]);
      ...
      <div className="hidden lg:flex flex-col w-full h-full">
        <div className="flex flex-row w-full lg:w-2/3 h-1/2 overflow-auto lg:overflow-hidden">
          <div className="flex flex-col space-y-10 mt-6 mr-5 ml-16 lg:ml-10 lg:space-y-0 lg:space-x-10 lg:flex-row lg:flex-wrap">
            <ArtCard key={artInfo.artName} art={artInfo} />
          </div>
        </div>
                ...
                <div className="mr-4 mt-10">
                  ...
                  <div className="mt-0 flex items-center justify-center w-full">
                    <div className="w-full text-center">
                      <h1 className="text-xl font-Antonio whitespace-nowrap overflow-hidden overflow-ellipsis">{artName}</h1>
                    </div>
                  </div>
                </div>
          ...
          <div id="details" className="flex flex-col justify-center items-center w-full">
            <div className="w-full flex flex-col justify-center items-center text-white">
              <div className="w-full flex flex-col items-center">
                <div className="flex items-center justify-center w-20 h-20">
                  <Image height={150} src={artImage} width={100} alt="artImage" />
                </div>
