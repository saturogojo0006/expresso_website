    ...
    <>
      <div className="parent">
        ...
        <div className={styles.parent3} style={{ height: parentHeight }}>
          <div className={`relative h-auto sm:h-96 mt-[-100px] about-section ${styles.description}`}>
            <div className="w-40 h-48 bg-orange-300 rounded-full mx-auto sm:ml-7 "></div>
            <div className={`absolute top-8 bg-slate-900 px-12 pt-6 pb-3 mx-6 rounded-3xl my-3 text-white flex flex-col sm:flex-row items-start gap-x-8 justify-between ${styles.bgclr}`}>
              ...
              <div className="mt-4 sm:mt-0 text-center sm:text-left">
                <p className={styles.para}>{isMobile && !showFullDescription ? shortText : fullText}</p>
                {isMobile && (
                  showFullDescription ? (
                    <span className={`read-less ${styles.readLess}`} onClick={handleReadLess}>
                      Read Less
                    </span>
                  ) : (
                    <span className={`read-more ${styles.readMore}`} onClick={handleReadMore}>
                      Read More
                    </span>
                  )
                )}
